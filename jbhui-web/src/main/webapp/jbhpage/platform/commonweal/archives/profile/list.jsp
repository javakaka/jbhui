	...
	<form id="listForm" action="list.do" method="get">
			...
			<div class="menuWrap">
				...
				<div class="popupMenu">
					<ul id="searchPropertyOption">
						...
						<li>
							<a href="javascript:;" <c:if test="${page.searchProperty == 'SCHOOL'}">class="current"</c:if> val="SCHOOL">学校</a>
						</li>
					</ul>
				</div>
			</div>
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="ICON_URL">头像</a>
				</th>
				<th>
				...
				<th>
					<a href="javascript:;" class="sort" name="DONATION_SOURCE">资助来源</a>
				</th>
				<th>
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					...
					<td>
						<img src="<%=imgBasePath%>${row.ICON_URL}" alt="${row.SUMMARY}" style="width:50px;height:50px;" />
					</td>
					<td>
						<span title="${row.NAME}">${row.NAME}</span>
					</td>
					<td>
						<span title="${row.SCHOOL}">${row.SCHOOL}</span>
					</td>
					<td>
					...
					<td>
						<span title="${row.DONATION_SOURCE}">${row.DONATION_SOURCE}</span>
					</td>
					<td>
						<span title="${row.DONATION_MONEY}">${row.DONATION_MONEY}</span>
					</td>
					<td>
						<c:choose>
