...
$().ready(function() {
	...
	function submit()
	{
		...
		$.ajax({
				url: "<%=basePath%>jbhpage/platform/goods/profile/saveDetail.do",
				type: "POST",
				data: params,
				dataType: "json",
				cache: false,
	...
	<input type="hidden" id="ID" name="ID" class="text" maxlength="200" value="${id}"/>
		<table class="input">
			...
			<tr>
				...
				<td>
					<input type="button" name="uploadBtn2" id="uploadBtn2" class="button" value="点击上传" onclick="upload2()"/>
				</td>
			</tr>
			<c:forEach items="${datail_dataset}" var="row" varStatus="status">
				<tr>
				<th>
				</th>
				<td>
...
function upload2()
{
	...
	deal_type ="goods_detail_icon";
	type ="goods_detail_icon";
	sub_type ="goods_detail_icon";
	url ="<%=basePath%>sysupload/Upload.jsp";
	url+="?deal_type="+deal_type;
	url+="&deal_code="+id;
	url+="&type="+type;
	url+="&sub_type="+sub_type;
	...
	url+="&file_type="+file_type;
	url+="&callback="+callback;
	window.open (url,'uploadwindow2','height=400,width=680,top=200,left=200 ,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
}
