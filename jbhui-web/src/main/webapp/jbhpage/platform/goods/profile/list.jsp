...
<body>
	...
	<form id="listForm" action="list.do" method="get">
		<div class="bar">
			<a href="edit_tab.jsp" class="iconButton">
			...
			<div class="menuWrap">
				...
				<div class="popupMenu">
					<ul id="searchPropertyOption">
						...
						<li>
							<a href="javascript:;" <c:if test="${page.searchProperty == 'TYPE_NAME'}">class="current"</c:if> val="TYPE_NAME">分类名称</a>
						</li>
					</ul>
				</div>
			</div>
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="TYPE_NAME">所属分类</a>
				</th>
				<th>
				...
				<th>
					<a href="javascript:;" class="sort" name="IS_COUPON">是否优惠</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="TOTAL_NUM">总数</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="LEFT_NUM">剩余</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="SALE_NUM">已售</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="SCORE">积分</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="HOT">首页</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="IS_TOP">置顶</a>
				</th>
				<th>
					<a href="javascript:;" class="sort" name="TRANS_FREE">包邮</a>
				</th>
				<th>
			...
			<c:forEach items="${page.content}" var="row" varStatus="status">
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${row.IS_COUPON == 1}">是</c:when>
							<c:when test="${row.IS_COUPON == 0}">否</c:when>
					...
					<td>
						<span title="${row.SALE_NUM}">${row.SALE_NUM}</span>
					</td>
					<td>
						<c:if test="${! empty row.SCORE }">
							<span title="${row.SCORE}">${row.SCORE}</span>
						</c:if>
					</td>
					<td>
						<c:choose>
							<c:when test="${row.HOT == 1}">是</c:when>
							<c:when test="${row.HOT == 0}">否</c:when>
							<c:otherwise>--</c:otherwise>
						</c:choose>
					</td>
					<td>
						<c:choose>
							<c:when test="${row.IS_TOP == 1}">是</c:when>
							<c:when test="${row.IS_TOP == 0}">否</c:when>
							<c:otherwise>--</c:otherwise>
						</c:choose>
					</td>
					<td>
						<c:choose>
							<c:when test="${row.TRANS_FREE == 1}">是</c:when>
							<c:when test="${row.TRANS_FREE == 0}">否</c:when>
							<c:otherwise>--</c:otherwise>
						</c:choose>
					</td>
					<td>
