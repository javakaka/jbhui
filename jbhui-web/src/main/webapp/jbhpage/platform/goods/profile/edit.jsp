		  ...
		  { 
			  ...
			  submitHandler: function(form) {  //通过之后回调 
				  ...
				  /* params=JSON.stringify(params); */
			  	  /* alert(params); */
					$.ajax({
							url: "<%=basePath%>jbhpage/platform/goods/profile/save.do",
							type: "post",
							data: params,
							dataType: "json",
							cache: false,
...
function selectTypeIcon(){
	...
	title ="选择商品类型";
	url="iframe:<%=basePath%>jbhpage/platform/goods/type/select.do?id="+id;
	width=620;
...
<body>
	...
	<input type="text" id="ID" name="ID" class="hidden" maxlength="200" value="${row.ID}" />
		<table class="input">
			...
			<tr>
				<th>
					<span class="requiredField">*</span>是否推荐到首页:
				</th>
				<td>
			...
			<tr>
				<th>
					<span class="requiredField">*</span>是否置顶(首页顶部推荐商品):
				</th>
				<td>
					<select id="IS_TOP" name="IS_TOP" style="width:190px;">
						<c:choose>
							<c:when test="${row.IS_TOP == 1}">
								<option value="1" selected>是</option>
								<option value="0" >否</option>
							</c:when>
			...
			<tr>
				<th>
					<span class="requiredField">*</span>是否包邮:
				</th>
				<td>
					<select id="TRANS_FREE" name="TRANS_FREE" style="width:190px;">
						<c:choose>
							<c:when test="${row.TRANS_FREE == 1}">
								<option value="1" selected>是</option>
								<option value="0" >否</option>
							</c:when>
							<c:when test="${row.TRANS_FREE == 0}">
								<option value="1" >是</option>
								<option value="0" selected>否</option>
							</c:when>
							<c:otherwise>
