...
{ 
	var result ="";
	var now = new Date(); 
	var values = end_date.split(/[^0-9]/);
	var fmtyear = parseInt(values[0], 10);
	var fmtmonth = parseInt(values[1], 10) - 1; // Month is zero based, so subtract 1
	var fmtday = parseInt(values[2], 10);
	var fmthours = parseInt(values[3], 10);
	var fmtminutes = parseInt(values[4], 10);
	var fmtseconds = parseInt(values[5], 10);
	var endDate = new Date(fmtyear, fmtmonth, fmtday, fmthours, fmtminutes, fmtseconds);
...
//计算当前时间和指定时间的差,返回小时，分钟,秒
function setIntevalWithHourMinuteSecond(dom_id,end_date) 
{ 
 ...
 * sp_time 指定的时间
 */
function showInterval(dom_id,sp_time,interval)
{
	if(typeof interval == "undefined" || interval == null || interval == 0)
	{
		interval =1000;
	}
	window.setInterval(function(){setIntevalWithHourMinuteSecond(dom_id,sp_time);}, interval);
}
...
sHeight=screen.height;//获取屏幕高度 
msgw =sWidth*0.6;
var tip_window_left_width =(sWidth - msgw)/2;
var bgObj=document.createElement("div");//关键在这里，原理：在body中创建一个div，并将其宽度与高度设置为覆盖整个窗体，如此一来就无法对其它窗口时行操作 
bgObj.setAttribute('id','bgDiv'); 
bgObj.style.position="absolute"; 
bgObj.style.top="0"; 
...
msgObj.style.border="1px solid " + bordercolor; 
msgObj.style.position = "absolute"; 
msgObj.style.left =tip_window_left_width+"px"; 
msgObj.style.top = "50%"; 
msgObj.style.font="12px/1.6em Verdana, Geneva, Arial, Helvetica, sans-serif"; 
//msgObj.style.marginLeft = "-225px" ; 
msgObj.style.marginTop = -75+document.documentElement.scrollTop+"px"; 
 ...
 * @returns
 */
function loadCookie( keyName )
{
	var cookieValue =$.cookie(keyName);
	return cookieValue;
}
...
function setCookie( keyName ,value , days)
{
	...
	$.cookie(keyName, value, { expires: days });
}
