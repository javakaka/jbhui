...
CDrag.Table.Rows.prototype = {
	...
	//初始化信息
		...
		if (wc.window == 0) {
			...
			wc.reduce.innerHTML = "<img src='"+WEB_PATH+"/images/ico_min_max.gif'>";
		} else {
			wc.content.style.display = "block";
			wc.reduce.innerHTML ="<img src='"+WEB_PATH+"/images/ico_min_max.gif'>";
		}
		
		wc.lock.innerHTML = !wc.locks ? "<img src='"+WEB_PATH+"/images/ico_lock.gif' alt='锁定' title='锁定'>" : "<img src='"+WEB_PATH+"/images/ico_lock.gif' alt='解锁' title='解锁'>";
	},
	...
	//获取相关信息
		...
		if (info.src) 
		{
			var iFrameId ="iframe_"+wc.root_id;
			wc.content.innerHTML = "<iframe height='208px' id='"+iFrameId+"'src='"+WEB_PATH+info.src+"' scrolling='no' frameborder=0 width='100%' onload='javascript:dyniframesize('"+iFrameId+"')'/>";
...
CDrag.Add.prototype = {
	...
	//初始化原始数据信息串
		var wc = this, parent = wc.parent,
			cols = parent.table.items, new_json = {}, r, i, j;
		var init_json = CDrag.database.json;
...
CDrag.prototype = {
	...
	//变大变小
		...
		if ((o.window = (o.window == 1 ? 0 : 1))) {
			...
			o.reduce.innerHTML = "<img src='"+WEB_PATH+"/images/ico_min_max.gif'>";
		} else {
	...
	//锁定
		...
		if (o.locks) {
			o.locks = false;
			o.lock.innerHTML = "<img src='"+WEB_PATH+"/images/ico_lock.gif' alt='锁定' title='锁定'>";
		} else {
			o.locks = true;
			o.lock.innerHTML = "<img src='"+WEB_PATH+"/images/ico_unlock.gif' alt='解锁' title='解锁'>";
		}
		wc.create_json();
	},
	...
	getConfig:function(sDeskConfig)
	{
		var wc = this;
		var s =null;
		...
		$.ajax({
			...
			success: function(ovo, textStatus) {
				...
				if(code >=0)
				{
					var part_serial =ovo.oForm.PART_SERIAL;
					...
					if(sConfig =="")
					sConfig =part_serial;
					var saC =sConfig.split("|");
					if(saC.length ==3)
					{
						s ="[";
						var sLeft ="{ cols : 'm_1', rows : [";
						if(saC[0] !="")
						{
							var saL =saC[0].split(",");
							for(var k=0; k<saL.length; k++)
							{
								if(k>0)
									sLeft +=",";
								sLeft +='{ id : "'+saL[k]+'", window : 1, locks : false }';
							}
						}
						sLeft +="] },";
						var sCenter ="{ cols : 'm_2', rows : [";
						if(saC[1] !="")
						{
							var saCe =saC[1].split(",");
							for(var k=0; k<saCe.length; k++)
							{
								if(k>0)
									sCenter +=",";
								sCenter +='{ id : "'+saCe[k]+'", window : 1, locks : false }';
							}
						}
						sCenter +="] },";
						var sRight =sRight ="{ cols : 'm_3', rows : [";
						if(saC[2] !="")
						{
							var saR =saC[2].split(",");
							for(var k=0; k<saR.length; k++)
							{
								if(k>0)
									sRight +=",";
								sRight +='{ id : "'+saR[k]+'", window : 1, locks : false }';
							}
						}
						sRight +="] }";
						s +=sLeft+sCenter+sRight+"]";
					}
					var oRs =list;
					var sJson ="[";
					if(oRs !=null)
					{
						$.each(oRs,function(i,item){
							if( i>0 )
							{
								sJson +=",";
							}
							sJson +='{id:"'+item.WEBPART_NO+'", title:"'+item.WEBPART_TITLE+'",className:"abc",src:"'+item.WEBPART_URL+'"}';
						});
//						var i=0;
//						for(; i<oRs.length; i++)
//						{
//							sJson +='{id:"'+oRs[i]["webpart_no"].get(i, "webpart_no")+'", title:"'+oRs.get(i, "webpart_title")+'",className:"abc",src:"'+oRs.get(i, "webpart_url")+'"},';
//						}
//						sJson +='{id:"'+oRs.get(i, "webpart_no")+'", title:"'+oRs.get(i, "webpart_title")+'",className:"abc",src:"'+oRs.get(i, "webpart_url")+'"},';
					}
					sJson +="]";
					CDrag.database.json =eval('('+sJson+')');
//					return s;
				}
				else
				{
...
Object.addEvent(window, ["onload"], viewDesktop);

function viewDesktop() {
	var wc = new CDrag, cookie = null;//wc.get_cookie();
	var s =wc.getConfig();
	json = eval('('+s+')');
	wc.parse(json);
	wc = null;
