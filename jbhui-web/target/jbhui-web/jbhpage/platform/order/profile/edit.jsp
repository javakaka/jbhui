	...
	<input type="hidden" name="ID" class="text" maxlength="200" value="${row.ID}"/>
		<table class="input">
			...
			<tr>
				...
				<td>
					<select id="DELIVERY_ID" name="DELIVERY_ID" class="text" style="width:190px;" >
					<option value="" >请选择... </option>
					<c:forEach items="${delivery_list}" var="delivery" varStatus="status">
						<option value="${delivery.ID }" <c:if test="${row.DELIVERY_ID == delivery.ID }"> selected </c:if> > ${delivery.NAME } </option>
					</c:forEach>
					</select>
				</td>
			</tr>
