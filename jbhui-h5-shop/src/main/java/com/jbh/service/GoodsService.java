...
public class GoodsService extends JdbcService{
	...
	public Row findDetailWithoutPicture(String id)
	{
		...
		String sSql ="select a.id,a.type_id,a.`name`,a.summary,a.weight,a.unit,"
				+ "a.raw_price,a.coupon_price,a.is_coupon,a.left_num,a.sale_num,a.score,a.detail,a.state,a.total_num, a.hot,a.is_top,a.trans_free, "
				+ "d.type_name "
				+" from hslg_goods a "
	...
	@SuppressWarnings("unchecked")
	public DataSet findDetailPictureList(String id)
	{
		DataSet ds =null;
		String sSql ="select c.FILE_PATH "
				+" from file_attach_control b  "
				+" left join file_attach_upload c on b.CONTROL_ID=c.CONTROL_ID "
				+" where  b.DEAL_CODE='"+id+"' and b.DEAL_TYPE='goods_detail_icon'";
		ds =queryDataSet(sSql);
		Setting setting =SettingUtils.get();
		String site_url =setting.getSiteUrl();
		for(int i=0;i<ds.size(); i++ )
	...
	public Page queryPage(Pageable pageable) {
		...
		+" a.total_num,a.left_num,a.sale_num,a.hot,a.team_buy,a.state,a.create_time,a.score,a.is_top,a.trans_free,b.type_name from hslg_goods a "
	...
	@SuppressWarnings("unchecked")
	public DataSet list(String type_id,String is_hot, String team_buy,
			String page,String page_size,String keyword,String order ,
			...
			)
	{
		...
		if(! StringUtils.isEmptyOrNull(transFree) )
		{
			sSql +=" and a.trans_free='"+transFree+"'";
		}
		if(! StringUtils.isEmptyOrNull(discount) )
		{
			sSql +=" and a.is_coupon='"+discount+"'";
		}
		sSql +=" group by a.id ";	
		if(! StringUtils.isEmptyOrNull(order) )
		{
			...
			if(order.equals("order_default")){
				sSql +=" order by  a.modify_time desc ";
			}
			//销量从高到低
			else if(order.equals("order_sale")){
				sSql +=" order by  a.sale_num desc ";
			}
			//价格从低到高
			else if(order.equals("order_price_up")){
				sSql +=" order by  a.raw_price  ";
			}
			//价格从高到低
			else if(order.equals("order_price_down")){
				sSql +=" order by  a.raw_price desc ";
			}
		}
	 ...
	 * @return
	 */
	public int getGoodsScoreByIds(String ids)
	{
		int sum =0;
		String sql =" select sum(score) from hslg_goods where id in ("+ids+")";
	...
	public DataSet queryTopGoods()
	{
		...
		+" left join file_attach_control b on b.DEAL_CODE=a.id and b.DEAL_TYPE='goods_icon' "
		+" left join file_attach_upload c on b.CONTROL_ID=c.CONTROL_ID "
		+" where 1=1 and a.state='1' and a.left_num > 0 and a.is_top =1 ";
		sSql +=" group by a.id  ";	
		sSql +=" order by a.modify_time desc  limit 0 , 10";	
		ds =queryDataSet(sSql);
		Setting setting =SettingUtils.get();
		String site_url =setting.getSiteUrl();
		if(ds != null && ds.size() >0 )
	...
	// limit 100
	@SuppressWarnings("unchecked")
	public DataSet queryIndexGoods()
	{
		DataSet ds =new DataSet();
		String sSql ="select a.id,a.type_id,a.`name`,a.summary,a.weight,a.unit,"
				+ "a.raw_price,a.coupon_price,a.is_coupon,a.left_num,a.sale_num,a.score,c.FILE_PATH "
				...
				+" left join file_attach_control b on b.DEAL_CODE=a.id and b.DEAL_TYPE='goods_icon' "
				+" left join file_attach_upload c on b.CONTROL_ID=c.CONTROL_ID "
				+" where 1=1 and a.state='1' and a.left_num > 0 and a.hot ='1' and a.is_top ='0'";
		sSql +=" group by a.id ";	
		sSql +=" order by a.modify_time desc  limit 0 , 100 ";	
		ds =queryDataSet(sSql);
		Setting setting =SettingUtils.get();
		String site_url =setting.getSiteUrl();
		if(ds != null && ds.size() >0 )
	...
	@SuppressWarnings("unchecked")
	public DataSet queryShopCartGoods(String ids)
	{
		DataSet ds =new DataSet();
		String sSql ="select a.id,a.type_id,a.`name`,a.summary,a.weight,a.unit,"
		+ "a.raw_price,a.coupon_price,a.is_coupon,a.left_num,a.sale_num,a.score,a.trans_free,c.FILE_PATH "
		+" from hslg_goods a "
		+" left join file_attach_control b on b.DEAL_CODE=a.id and b.DEAL_TYPE='goods_icon' "
		+" left join file_attach_upload c on b.CONTROL_ID=c.CONTROL_ID "
