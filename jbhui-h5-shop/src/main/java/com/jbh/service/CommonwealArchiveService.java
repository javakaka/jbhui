...
@Component("hslgCommonwealArchiveService")
public class CommonwealArchiveService extends JdbcService{

	public CommonwealArchiveService() {
	...
	{
		Row row =null;
		String sSql ="select a.* from hslg_commonweal_archives a "
				+" where a.id ='"+id+"' ";
		row =queryRow(sSql);
		return row;
	}
	...
	{
		int num =0;
		int id =getTableSequence("hslg_commonweal_archives", "id", 1);
		row.put("id", id);
		row.put("create_time", DateUtil.getCurrentDateTime());
		num =insert("hslg_commonweal_archives", row);
		return num;
	}
	...
	public int update(Row row)
	{
		...
		Assert.notNull(id);
		num =update("hslg_commonweal_archives", row, " id='"+id+"'");
		return num;
	}
	...
	public Page queryPage() {
		...
		String sql ="select a.* from hslg_commonweal_archives a  "
		+" where 1=1 ";
		String restrictions = addRestrictions(pageable);
		String orders = addOrders(pageable);
		sql += restrictions;
		sql += orders;
		String countSql ="select count(*) from hslg_commonweal_archives "
		+" where 1=1 ";
	...
	public DataSet list(String page, String page_size)
	{
		...
		String sql ="select id,name,summary,address,school,icon_url,donation_source,donation_money,state from hslg_commonweal_archives   "
	...
	{
		DataSet ds =new DataSet();
		String sSql =" select id,type_name from hslg_commonweal_archives order by level_index ";
	...
	public void delete(String... ids) {
		...
		if (ids != null) {
			...
			sql = "delete from hslg_commonweal_archives where id in(" + id + ")";
			update(sql);
		}
	}
	...
	{
		boolean bool =true;
		String sql ="select count(*) from hslg_commonweal_archives where type_name ='"+name+"'";
	...
	{
		boolean bool =true;
		String sql ="select count(*) from hslg_commonweal_archives where type_name ='"+name+"' and id !='"+id+"'";
		String count =queryField(sql);
		int sum =Integer.parseInt(count);
		if(sum >0)
			bool =false;
