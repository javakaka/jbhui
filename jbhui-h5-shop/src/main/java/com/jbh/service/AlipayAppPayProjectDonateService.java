 ...
 */
@Component("hslgAlipayAppPayProjectDonateService")
public class AlipayAppPayProjectDonateService extends JdbcService {

	private static Logger logger = Logger.getLogger(AlipayAppPayProjectDonateService.class);
	...
	public AlipayAppPayProjectDonateService() 
	{
	...
	public OVO validate(IVO ivo) throws Exception
	{
		...
		Row archiveRow =commonwealProjectService.find(order_id);
		if(archiveRow == null)
		{
			ovo =new OVO(-20012,"捐赠所属项目不存在","捐赠所属项目不存在");
			return ovo;
		}
		String order_no="";
		...
		orderRow.put("project_id", order_id);
		...
		commonwealProjectRecordService.insert(orderRow);
		Setting setting =SettingUtils.get();
		String site_url =setting.getSiteUrl();
		String notify_url =site_url+"/api/pay/alipay/app/notifyProjectDonate.do";
		ovo =new OVO(0,"","");
