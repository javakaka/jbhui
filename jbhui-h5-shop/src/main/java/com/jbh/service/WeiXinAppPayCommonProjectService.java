 ...
 */
@Component("hslgWeiXinAppPayProjectService")
public class WeiXinAppPayCommonProjectService extends JdbcService {

	private static Logger logger = Logger.getLogger(WeiXinAppPayCommonProjectService.class);
	...
	private CommonwealProjectRecordService orderService;
	
	public WeiXinAppPayCommonProjectService() 
	{
 ...
 */
	public OVO validate(IVO ivo) throws Exception
	{
		...
		String project_id =ivo.getString("order_id");//公益项目ID
		...
		if(StringUtils.isEmptyOrNull(project_id))
		{
			ovo =new OVO(-20011,"公益项目ID不能为空","公益项目ID不能为空");
			return ovo;
		}
		Row archiveRow =commonwealProjectService.find(project_id);
		if(archiveRow == null)
		{
			ovo =new OVO(-20012,"捐赠所属公益项目不存在","捐赠所属公益项目不存在");
			return ovo;
		}
		String order_no="";
		String cur_time =DateUtil.getCurrentDateTime().replace(" ", "").replace(":", "").replace("-", "");
		Random random =new Random(100);
		int rand_int =random.nextInt(1000)%(1000-100+1)+100;
		//倒数第三第四为定义：01 爱心捐赠／善小档案捐赠，02公益项目捐赠
		order_no =cur_time+"02"+String.valueOf(rand_int);
		Row orderRow =new Row();
		orderRow.put("user_id", user_id);
		orderRow.put("project_id", project_id);
		...
		orderRow.put("pay_type", "1");//1微信2支付宝3银联
...
//		String body ="订单:"+order_no+"支付备注";
		String body ="好善乐购公益项目捐赠订单:"+order_no;
		...
		String notify_url =site_url+"/api/pay/weixin/app/notify-project.do";
		...
		//设置API_KEY 
		ovo.set("api_key", "haoshanlegouhaoshanlegou2015tong");
		//公益项目欲付单处理成功 设置订单的状态0待支付1已支付未到账2支付完成
	...
	public OVO parsePreOrderResult(String xml,String device) throws JException
	{
		...
		{
			ovo =new OVO(0,"公益项目欲付单处理成功","公益项目欲付单处理成功");
...
//			ovo.set("package", "Sign=WXPay");
		...
		{
			ovo =new OVO(-10030,"公益项目欲付单处理失败","公益项目欲付单处理失败");
			ovo.set("result", "fail");
