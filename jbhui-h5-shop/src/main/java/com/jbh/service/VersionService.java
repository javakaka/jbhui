 ...
 * 类说明:广告业务处理类 
 */
@Component("hslgVersionService")
public class VersionService extends JdbcService {
	...
	{
		int rowNum =0;
		int id =getTableSequence("hslg_app_version", "id", 1);
		row.put("id", id);
		row.put("create_time", DateUtil.getCurrentDateTime());
		rowNum =insert("hslg_app_version", row);
		return rowNum;
	}
	...
	{
		Row row =null;
		sql ="select * from hslg_app_version where id='"+id+"'";
		row =queryRow(sql);
		return row;
	}
	...
	public int update(Row row)
	{
		...
		update("hslg_app_version", row, " id='"+id+"'");
		return rowNum;
	}
	...
	public DataSet list()
	{
		...
		sql ="select id,name,picture,width,height from hslg_app_version where status=1 and (start_time<='"+now_time+"' and end_time>='"+now_time+"')";
		ds =queryDataSet(sql);
		return ds;
	...
	public void delete(Long... ids) {
		...
		if (ids != null) {
			...
			sql = "delete from hslg_app_version where id in(" + id + ")";
			update(sql);
		}
	}
	...
	{
		//update db record
		String sql =" update hslg_app_version set is_curhslg_version='2' where app_id='"+app_id+"' and device='"+device+"'";
		update(sql);
		sql ="update hslg_app_version set is_curhslg_version='1' where id='"+version_id+"'";
		update(sql);
