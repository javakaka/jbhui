...
@Component("hslgConvenientInfoService")
public class ConvenientInfoService extends JdbcService{

	public ConvenientInfoService() {
	...
	{
		Row row =null;
		String sSql ="select a.*,b.name as type_name from hslg_convenient_info a left join hslg_convenient_type b on a.type_id=b.id "
				+" where a.id ='"+id+"' ";
		row =queryRow(sSql);
		return row;
	}
	...
	{
		int num =0;
		int id =getTableSequence("hslg_convenient_info", "id", 1);
		row.put("id", id);
		row.put("create_time", DateUtil.getCurrentDateTime());
		num =insert("hslg_convenient_info", row);
		return num;
	}
	...
	public int update(Row row)
	{
		...
		Assert.notNull(id);
		num =update("hslg_convenient_info", row, " id='"+id+"'");
		return num;
	}
	...
	public Page queryPage() {
		...
		String sql ="select * from (select a.*,b.name as type_name from hslg_convenient_info a left join hslg_convenient_type b on a.type_id=b.id ) as tab "
		+" where 1=1 ";
		String restrictions = addRestrictions(pageable);
		String orders = addOrders(pageable);
		sql += restrictions;
		sql += orders;
		String countSql ="select count(*) from (select a.*,b.name as type_name from hslg_convenient_info a left join hslg_convenient_type b on a.type_id=b.id ) as tab "
		+" where 1=1 ";
	...
	public DataSet list(String page, String page_size,String type_id)
	{
		...
		if(ds != null )
		{
			...
			int iPos =domain.lastIndexOf("/");
			if(iPos != -1)
			{
				domain =domain.substring(0,iPos);
			}
	...
	{
		DataSet ds =new DataSet();
		String sSql =" select id,type_name from hslg_convenient_info order by level_index ";
	...
	public void delete(String... ids) {
		...
		if (ids != null) {
			...
			sql = "delete from hslg_convenient_info where id in(" + id + ")";
			update(sql);
		}
	}
	...
	{
		boolean bool =true;
		String sql ="select count(*) from hslg_convenient_info where type_name ='"+name+"'";
	...
	{
		boolean bool =true;
		String sql ="select count(*) from hslg_convenient_info where type_name ='"+name+"' and id !='"+id+"'";
		String count =queryField(sql);
		int sum =Integer.parseInt(count);
		if(sum >0)
			bool =false;
