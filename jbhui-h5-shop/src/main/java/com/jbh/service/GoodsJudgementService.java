...
public class GoodsJudgementService extends JdbcService{
	 ...
	 */
	@Transactional(value="jdbcTransactionManager",propagation=Propagation.REQUIRED)
	public int findGoodsTotalJudgeNum(String id) {
		int num =0;
		String sql = "select count(*) from hslg_goods_judgement where goods_id ='"+ id +"'";
	 ...
	 */
	@Transactional(value="jdbcTransactionManager",propagation=Propagation.REQUIRED)
	public int findGoodsTotalJudgeNumByUserId(String userId) {
		int num =0;
		String sql = "select count(*) from hslg_goods_judgement where user_id ='"+ userId +"'";
		num =Integer.parseInt(queryField(sql));
		return num;
	}
