...
public class OrderItemService extends JdbcService{
	...
	public DataSet findOrderItems(String order_id)
	{
		...
		String sSql ="select a.*,b.`name` as goods_name,fu.file_path from hslg_order_item a "
				...
				+" left join file_attach_control fc on fc.DEAL_CODE=a.goods_id and fc.DEAL_TYPE='goods_icon' " 
	...
	@Transactional(value="jdbcTransactionManager",readOnly = true)
	public DataSet queryItemsByOrderId(String order_id) {
		String sql =" select * from "
				+" ( "
				+" select a.*,b.order_no,c.name as goods_name from hslg_order_item a " 
				+" left join hslg_order b on a.order_id=b.id "
	 ...
	 */
	@Transactional(value="jdbcTransactionManager",readOnly = true)
	public Page queryReport(String date,Pageable pageable) {
		Page page = null;
		String sql ="select * from ( select goods_id,goods.`name`, money,num from "
				+" ( "
				+" select goods_id, sum(goods_money) as money,sum(goods_num) as num from " 
				+" ( "
				+" select a.*,b.create_time from hslg_order_item a " 
				+" inner join hslg_order b on a.order_id=b.id  "
				+" where b.create_time like '%"+date+"%' and b.state !='0' "
				+" ) as tab  group by goods_id "
				+" ) as tab2  left join hslg_goods goods on tab2.goods_id=goods.id "
				+" order by money desc ) as temp_tab"
				+" where 1=1  ";
		String restrictions = addRestrictions(pageable);
		String orders = addOrders(pageable);
		sql += restrictions;
		sql += orders;
		String countSql ="select count(*) from (select * from "
				+" ( "
				+" select goods_id, sum(goods_money) as money,sum(goods_num) as num from " 
				+" ( "
				+" select a.*,b.create_time from hslg_order_item a " 
