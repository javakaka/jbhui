...
import com.ezcloud.framework.util.Base64Util;
...
//		}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value ="/score-collect")
	public @ResponseBody
	String queryUserScoreAndCollectSum(HttpServletRequest request) throws Exception
	{
		parseRequest(request);
		//用户ID
		String id =ivo.getString("id",null);
		if(StringUtils.isEmptyOrNull(id))
		{
			ovo =new OVO(0,"操作成功","操作成功");
			ovo.set("score", "0");
			ovo.set("collect_num", "0");
			return AesUtil.encode(VOConvert.ovoToJson(ovo));
		}
		Row userRow =userService.find(id);
		String score ="0";
		if(userRow != null )
		{
			score =userRow.getString("score","0");
			if(StringUtils.isEmptyOrNull(score))
			{
				score ="0";
			}
		}
		int collectNum =userCollectionService.getUserCollectionNum(id);
//		userCollectionService
		ovo =new OVO(0,"操作成功","操作成功");
		ovo.set("score", score);
		ovo.set("collect_num", collectNum);
		return AesUtil.encode(VOConvert.ovoToJson(ovo));
	}
