...
//		int order_num =orderService.getOrderNumByCreateTime(cur_time);
		int order_num =orderService.getOrderNumByDay(cur_time);
		if(order_num ==0)
		{
			order_num =10001;
		}
		...
		int score_sum =calculateOrderScoreSum(item_list);
		orderRow.put("score", score_sum);
...
//			保存订单项到数据库
	...
	String find(HttpServletRequest request) throws Exception
	{
		...
		String transfer_finish_time =orderRow.getString("transfer_finish_time","");//0未配送/已下单1已配送/配送中2已完成
		...
		ovo.set("transfer_finish_time", transfer_finish_time);
		ovo.set("order_no", order_no);
...
//		shopService.delete(id);
	 ...
	 * @return
	 */
	public int calculateOrderScoreSum(DataSet ds)
	{
		int sum =0;
		String ids ="";
		for(int i=0;i<ds.size(); i++)
		{
			Row item =(Row)ds.get(i);
			String id =item.getString("goods_id","");
			if(! StringUtils.isEmptyOrNull(id))
			{
				if(ids.length() > 0 )
				{
		...
		//11
		sum =goodsService.getGoodsScoreByIds(ids);
		return sum;
	}
	
	/**
	 * 查询下单发货开始时间
	 * 查询配置的每小时下单阀值，未超出阀值，取当前的小时，超出阀值，则将开始发货时间移到下一个小时
	 * @param request
	 * @return
	 * @throws Exception
	 */
	@RequestMapping(value ="/query-send-time")
	public @ResponseBody
	String queryStartSendTime(HttpServletRequest request) throws Exception
	{
		parseRequest(request);
		String begin_send_time ="";
		logger.info("查询下单发货开始时间");
		String[] arr =systemConfigService.getConfigData("HSLG_ORDER", "PER_HOUR_ORDER_NUM");
		String num ="0";
		if(arr != null)
		{
			num =arr[0];
		...
		//取当前小时的订单数量
		String cur_time =DateUtil.getCurrentDateTime();
		String cur_hour =cur_time.substring(0, 13);//2015-10-11 12:00:00
		int cur_order_hour_num =orderService.getOrderNumByHour(cur_hour);
		int limit_order_hour_num =Integer.parseInt(num);
		if(cur_order_hour_num <= limit_order_hour_num)
		{
			begin_send_time =cur_hour;
		}
		else
		{
			begin_send_time =DateUtil.getNextHourTime(cur_hour+":00:00");
		}
		ovo =new OVO(0,"","");
		ovo.set("begin_send_time", begin_send_time);
		return AesUtil.encode(VOConvert.ovoToJson(ovo));
	}
